...
```bash
$ git clone https://github.com/sebboer/restic-systemd-automatic-backup.git
$ cd restic-systemd-automatic-backup
$ sudo make install
````

### 1. Create S3 bucket for restic

Follow the instructions on the restic [documentation](https://restic.readthedocs.io/en/latest/080_examples.html) for creating an S3 bucket for restic.

### 2. Configure your AWS account locally
Put these files in `/etc/restic/`:
* `env.sh`: Fill this file out with your AWS Keys and bucket location. The reason for putting these in a separate file is that it can be used also for you to simply source, when you want to issue some restic commands. For example:
```bash
